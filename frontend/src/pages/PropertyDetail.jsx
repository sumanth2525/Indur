...
import { trackContactSeller, trackPropertyView, trackSaveProperty } from '../services/analytics'
import {
...
export default function PropertyDetail() {
  ...
  if (!property) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-5">
        ...
        <button type="button" onClick={() => navigate('/browse')} className="mt-4 text-sm font-medium underline">
          {t('back')}
        </button>
      </div>
    )
  ...
  const isSaved = authenticated && user?.saved?.includes(property.id)
  ...
  const sellerPhone = guest ? null : seller?.phone
  ...
  const handleSave = async () => {
    ...
    if (!user?.id) return
    const saved = !isSaved
    await toggleSavedListing(user.id, property.id)
    trackSaveProperty(property.id, saved)
    refreshUser()
  }
        ...
        <div className="relative aspect-[16/10] lg:aspect-[16/9] bg-surface">
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-2">
            ...
            <button
              type="button"
              onClick={handleSave}
              className={`rounded-lg border p-1.5 shadow-sm transition-colors ${
